...
<Note>
  Dachser is queried via its live API and requires a valid `DACHSER_API_KEY` in your environment. If the key is absent, Dachser is skipped and no error is raised for the other carriers. DHL and FedEx API key variables are reserved in the configuration for future adapters.
</Note>
...
Agencies are typed as one of three values defined in the codebase:
