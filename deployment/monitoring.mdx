---
title: "Ship Quote Observability: Winston, Loki, and Grafana Logs"
...
- **Log-level routing:** The severity is derived automatically from the HTTP status code via `getLogLevelFromStatus`. `2xx/3xx` → `info`, `4xx` → `warn`, `5xx` → `error`. This means failed requests surface immediately in Grafana when filtering on `level=warn` or `level=error`.
